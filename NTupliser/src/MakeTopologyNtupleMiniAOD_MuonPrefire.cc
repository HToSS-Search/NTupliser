...
MakeTopologyNtupleMiniAOD_MuonPrefire::MakeTopologyNtupleMiniAOD_MuonPrefire(
    ...
    , conversionsToken_{consumes<reco::ConversionCollection>(iConfig.getParameter<edm::InputTag>("conversionsToken"))}
...
//    , ecalBadCalibFilterUpdate_token_{mayConsume<bool>(iConfig.getParameter<edm::InputTag>("ecalBadCalibReducedMINIAODFilter"))}
    ...
    , invMuMuMassCutHi_{iConfig.getParameter<double>("maxInvMuMuMass")}
    , invMuMuMassCutLo_{iConfig.getParameter<double>("minInvMuMuMass")}
    , invChChMassCutHi_{iConfig.getParameter<double>("maxInvChChMass")}
    , invMuMuhhMassCut_{iConfig.getParameter<double>("minInvMuMuhhMass")}
    , muoDRCut_{iConfig.getParameter<double>("maxMuonDeltaR")}
    , muoIsoCut_{iConfig.getParameter<double>("muoRelIso")}
    , chPtCut_{iConfig.getParameter<double>("minHadronPt")}
    , chEtaCut_{iConfig.getParameter<double>("maxHadronEta")}
    ...
    // , fsrConLo{1.0}
{
    ...
    histocontainer1D_["eventFilterOR"]->GetXaxis()->SetBinLabel(2, "Trigger Cut");
    histocontainer1D_["eventFilterOR"]->GetXaxis()->SetBinLabel(3, "MET Filters");
    histocontainer1D_["eventFilterOR"]->GetXaxis()->SetBinLabel(4, "#mu selection");
    histocontainer1D_["eventFilterOR"]->GetXaxis()->SetBinLabel(5, "dimuon candidate");
    histocontainer1D_["eventFilterOR"]->GetXaxis()->SetBinLabel(6, "hadron trk selection");
    histocontainer1D_["eventFilterOR"]->GetXaxis()->SetBinLabel(7, "dihadron candidate");
    histocontainer1D_["eventFilterOR"]->GetXaxis()->SetBinLabel(8, "m(#mu#muhh) cut");
    histocontainer1D_["eventFilterAND"]->GetXaxis()->SetBinLabel(1, "TotalEvts");
    histocontainer1D_["eventFilterAND"]->GetXaxis()->SetBinLabel(2, "Trigger Cut");
    histocontainer1D_["eventFilterAND"]->GetXaxis()->SetBinLabel(3, "MET Filters");
    histocontainer1D_["eventFilterAND"]->GetXaxis()->SetBinLabel(4, "#mu selection");
    histocontainer1D_["eventFilterAND"]->GetXaxis()->SetBinLabel(5, "dimuon candidate");
    histocontainer1D_["eventFilterAND"]->GetXaxis()->SetBinLabel(6, "hadron trk selection");
    histocontainer1D_["eventFilterAND"]->GetXaxis()->SetBinLabel(7, "dihadron candidate");
    histocontainer1D_["eventFilterAND"]->GetXaxis()->SetBinLabel(8, "m(#mu#muhh) cut");
    if (isMC_){
        // histocontainer1D_["eventFilterOR_wtd"] = fs->make<TH1D>("eventFilterOR_wtd", "eventFilterOR with wts", 8, 0.5, 8.5);
        // histocontainer1D_["eventFilterOR_wtd"]->GetXaxis()->SetBinLabel(1, "TotalEvts");
        // histocontainer1D_["eventFilterOR_wtd"]->GetXaxis()->SetBinLabel(2, "Trigger Cut");
        // histocontainer1D_["eventFilterOR_wtd"]->GetXaxis()->SetBinLabel(3, "MET Filters");
        // histocontainer1D_["eventFilterOR_wtd"]->GetXaxis()->SetBinLabel(4, "#mu selection");
        // histocontainer1D_["eventFilterOR_wtd"]->GetXaxis()->SetBinLabel(5, "dimuon candidate");
        // histocontainer1D_["eventFilterOR_wtd"]->GetXaxis()->SetBinLabel(6, "hadron trk selection");
        // histocontainer1D_["eventFilterOR_wtd"]->GetXaxis()->SetBinLabel(7, "dihadron candidate");
        // histocontainer1D_["eventFilterOR_wtd"]->GetXaxis()->SetBinLabel(8, "m(#mu#muhh) cut");
        histocontainer1D_["eventFilterAND_wtd"] = fs->make<TH1D>("eventFilterAND_wtd", "eventFilterAND with wts", 8, 0.5, 8.5);
        histocontainer1D_["eventFilterAND_wtd"]->GetXaxis()->SetBinLabel(1, "TotalEvts");
        histocontainer1D_["eventFilterAND_wtd"]->GetXaxis()->SetBinLabel(2, "Trigger Cut");
        histocontainer1D_["eventFilterAND_wtd"]->GetXaxis()->SetBinLabel(3, "MET Filters");
        histocontainer1D_["eventFilterAND_wtd"]->GetXaxis()->SetBinLabel(4, "#mu selection");
        histocontainer1D_["eventFilterAND_wtd"]->GetXaxis()->SetBinLabel(5, "dimuon candidate");
        histocontainer1D_["eventFilterAND_wtd"]->GetXaxis()->SetBinLabel(6, "hadron trk selection");
        histocontainer1D_["eventFilterAND_wtd"]->GetXaxis()->SetBinLabel(7, "dihadron candidate");
        histocontainer1D_["eventFilterAND_wtd"]->GetXaxis()->SetBinLabel(8, "m(#mu#muhh) cut");
    }
    // OR filter would count events that pass that particular filter
    // AND filter would count events that pass that particular filter and everything before it
...
void MakeTopologyNtupleMiniAOD_MuonPrefire::fillPV(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    if (pvHandle.isValid()) {
        ...
        for (auto it{pv.begin()}; it != pv.end() && numPVs < numeric_cast<int>(NPVSMAX); it++) {
            if ((PV_flag)&&(it->isValid() && it->chi2() != 0 && it->ndof() != 0 )) {
    ...
    for (reco::GenParticleCollection::const_iterator it = genParticles->begin(); it != genParticles->end(); ++it) {
        ...
        if (nGenPar < NGENPARMAX) {
            ...
            if (it->numberOfDaughters() > 1) {
                for (reco::GenParticleCollection::const_iterator mit = genParticles->begin(); mit != genParticles->end(); ++mit) {
                    if(it->daughter(1) == &(*mit)) { //it should daughter(1)
...
void MakeTopologyNtupleMiniAOD_MuonPrefire::fillTriggerData(const edm::Event& iEvent)
{
    ...
    iEvent.getByToken(trigToken_, hltResults);
		...
		//bool MutrigFlag = false;
		//bool IsoMutrigFlag = false;
    ...
    if (hltResults.product()->wasrun())
    {
        ...
        nTriggerBits = 0;
				//int trig_count=0; TString trig_name="HLT_Mu50_v"; 
        ...
        for (int itrig{0}; itrig < (int)hltnames_.size(); ++itrig)  {
            ...
            if (hltResults->error(itrig))
                trigbit = -2;
            TString trigPath = hltnames_[itrig];
						//if (trigPath.Contains("HLT_IsoMu27_v") && accept) IsoMutrigFlag=true;
						//if (trigPath.Contains(trigName_test) && accept) MutrigFlag=true;
            for (size_t iTrigList{0}; iTrigList < triggerList_.size(); iTrigList++) {
                TString trigTest = triggerList_[iTrigList];
                if (mytree_->GetEntries()<2) std::cout<<"trigger bits:"<<trigPath<<std::endl;
                if (trigPath.Contains(trigTest)) {
                //if (triggerList_[iTrigList] == hltnames_[itrig]) {
                    if (mytree_->GetEntries() < 10)
                    {
                         std::cout << "found 'standard' trigger bit "
                                   << trigPath << " FIRED:" << accept << std::endl;
                    }
		...
		//if (MutrigFlag && !IsoMutrigFlag) {std::cout<<"Found event"<<std::endl;trigCount++;}  
		//if (MutrigFlag) {std::cout<<"Found event with HLT_Mu50"<<std::endl;}  
		//if (IsoMutrigFlag) {std::cout<<"Found event with HLT_IsoMu27"<<std::endl;}  
...
//     // std::cout << "passes PUReweight" << std::endl;
...
void MakeTopologyNtupleMiniAOD_MuonPrefire::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    if ((!isggH_)) {
        ...
        if(genEventInfo.isValid()) {
            ...
            weight_ = 0; //NOTE: Weights from scale variations, PDFs etc. are stored in the relative product. Notice that to be used they need to be renormalized to the central event weight at LHE level which may be different from genEvtInfo->weight()
            processPtHat_=0;
            processId_=0;
        }
        if (debugMode_) std::cout << "passes nominal weight:"<<weight_ << std::endl;
        ...
        // if (isLHEflag_) {
            // std::cout << "passes LHEflag" << std::endl;
        
        iEvent.getByToken(externalLHEToken_, EventHandle);
        if (EventHandle.isValid()) {
            if (debugMode_) std::cout << "passes EventHandle" << std::endl;
    ...
    if ((isggH_)) {
        ...
        if (hadronType_ == "ChargedKaon") {
            ...
            if (( genPionIndex.size() >= 2 )&&(genMuonIndex.size() >= 2)) passGenCuts = true;
			  ...
			  else if (hadronType_ == "ChargedPionOnly") {
            if ( genPionIndex.size() >= 4) passGenCuts = true;
        }
				else if (hadronType_ == "ChargedKaonOnly") {
					  if ( genChargedKaonIndex.size() >= 4 ) passGenCuts = true;
				}
        else {
        ...
        if (debugMode_) std::cout << "passes eventcount" << std::endl;
            ...
            iEvent.getByToken(externalLHEToken_, EventHandle);
                // if (EventHandle.isValid()) std::cout << "passes EventHandle" << std::endl;
        ...
        // std::cout << "passes passMuonCuts && passGenCuts:" <<passMuonCuts<<","<<passGenCuts << std::endl;
    ...
    if (debugMode_) std::cout << "Filled Trigger" << std::endl;

    fillEventInfo(iEvent, iSetup);
    if (debugMode_) std::cout << "Filled EventInfo" << std::endl;

    fillPV(iEvent,iSetup);
    fillSV(iEvent,iSetup);
    if (debugMode_) std::cout << "Filled PV,SV" << std::endl;
...
// //    fillLostTracksCands(iEvent, iSetup);
    fillPackedCands(iEvent, iSetup);
    if (debugMode_) std::cout << "Filled packedCands" << std::endl;

    if (debugMode_) std::cout << "done with trigger and beam spot" << std::endl;
    ...
    if (debugMode_) std::cout << "Filled muons, MET" << std::endl;

    fillSummaryVariables();
    if (debugMode_) std::cout << "Filled Summary" << std::endl;
    ...
    if (!doCuts_) {
        ...
        if (Skim_=="ZJets") {
            ...
            for (int j{0}; j < numMuo["PF"]; j++) {
                if ( muonSortedPt["PF"][j] >= 10) muons.emplace_back(j); //storing indices
                if (muons.size()>=2) {
                    passMuonCuts=true;
                    break;
                }
            ...
            // if (muons.size()>=2) passMuonCuts=true;
            muons.clear();

            bool passDimuonCuts{false};
            for (int j{0}; j < numMuo["PF"]; j++) {
                if ( muonSortedPt["PF"][j] < 20) continue;
                if (!muonSortedTightCutId["PF"][j]) continue;
                // if ( std::abs(muonSortedEta["PF"][j]) > muoEtaCut_ ) continue;
                for (int k{j+1}; k < numMuo["PF"]; k++) {
                    if (!muonSortedTightCutId["PF"][k]) continue;
                    if ( muonSortedPt["PF"][k] < 20) continue;
                    ...
                    if ((invMass >= 50) && (invMass <= 130)) {
                        passDimuonCuts = true;
                        break;
                    }
                }
                if ( passDimuonCuts ) break;
            }
            // std::cout<<"Inside ZJets skim"<<std::endl;
            if ((passMuonCuts)&&(passDimuonCuts)) mytree_->Fill();
        }
        else if (Skim_=="ZH_production") {
            if (debugMode_) std::cout<<"Enters ZH skim"<<std::endl;
            ...
            // if (isMC_) {
            //     histocontainer1D_["eventFilterOR_wtd"]->Fill(1,weight_);
            // }
            
            bool passTrigCuts {true};
            //bool passTrigCuts {false};
            for (size_t iTrig{0}; iTrig < triggerList_.size(); iTrig++) {
                if (triggerRes[iTrig]==1) {
                    passTrigCuts = true;
                    break;
                }
            }
            if (debugMode_) std::cout<<"Passed TrigCuts"<<std::endl;

            if ((passGenCuts)&&(passTrigCuts)) {
                histocontainer1D_["eventFilterOR"]->Fill(2);
                // if (isMC_) histocontainer1D_["eventFilterOR_wtd"]->Fill(2,weight_);
            }
            if ((passGenCuts)&&(passTrigCuts)) { //no cut before this
                histocontainer1D_["eventFilterAND"]->Fill(2);
                if (isMC_) histocontainer1D_["eventFilterAND_wtd"]->Fill(2,weight_);
            }
            else return;
            if (debugMode_) std::cout<<"Passed GenCuts"<<std::endl;

            
            bool passMetFilters {true};
            for (size_t iMetFilterList{0}; iMetFilterList < metFilterList_.size(); iMetFilterList++) {
                if (!(metFilterRes[iMetFilterList]==1)) {
                    passMetFilters = false;
                    break;
                }
            }
            if (debugMode_) std::cout<<"Passed MetFilters"<<std::endl;

            if ((passGenCuts)&&(passMetFilters)) {
                histocontainer1D_["eventFilterOR"]->Fill(3);
                // if (isMC_) histocontainer1D_["eventFilterOR_wtd"]->Fill(3,weight_);
            }
            if ((passGenCuts)&&(passTrigCuts)&&(passMetFilters)) { //no cut before this
                histocontainer1D_["eventFilterAND"]->Fill(3);
                if (isMC_) histocontainer1D_["eventFilterAND_wtd"]->Fill(3,weight_);
            }
            else return;
            ...
            std::vector<int> muons;
            for (int j{0}; j < numMuo["PF"]; j++) {
                if ( std::abs(muonSortedEta["PF"][j]) <= muoEtaCut_ ) {
                    if ( muonSortedPt["PF"][j] > (muons.empty() ? muoPt1Cut_ : muoPt2Cut_)) muons.emplace_back(j); //storing indices
                }
                if (muons.size()>=2) {
                    passMuonCuts=true;
                    break;
            ...
            // if (muons.size()>=2) passMuonCuts=true;
            muons.clear();
            if ((passGenCuts)&&(passMuonCuts)) {
                histocontainer1D_["eventFilterOR"]->Fill(4);
                // if (isMC_) histocontainer1D_["eventFilterOR_wtd"]->Fill(4,weight_);
            }
            if ((passGenCuts)&&(passTrigCuts)&&(passMetFilters)&&(passMuonCuts)) { //no cut before this
                histocontainer1D_["eventFilterAND"]->Fill(4);
                if (isMC_) histocontainer1D_["eventFilterAND_wtd"]->Fill(4,weight_);
            }
            else return;

            if (debugMode_) std::cout<<"Passed Muon Cuts"<<std::endl;
            ...
            for (int j{0}; j < numMuo["PF"]; j++) {
                ...
                for (int k{j+1}; k < numMuo["PF"]; k++) {
                    ...
                    // }
                    // if (muon1.DeltaR(muon2) >= muoDRCut_) continue;
                    double invMass { (muon1+muon2).M() };
                    if ((invMass <= invMuMuMassCutHi_) && (invMass >= invMuMuMassCutLo_)) {
                        passDimuonCuts = true;
                        break;
                    }
                }
            ...
            if (debugMode_) std::cout<<"No. of dimuon iters:"<<dimu_iters<<std::endl;
            if ((passGenCuts)&&(passDimuonCuts)) {
                histocontainer1D_["eventFilterOR"]->Fill(5);
                // if (isMC_) histocontainer1D_["eventFilterOR_wtd"]->Fill(5,weight_);
            }
            if ((passGenCuts)&&(passTrigCuts)&&(passMetFilters)&&(passMuonCuts)&&(passDimuonCuts)) { //no cut before this
                histocontainer1D_["eventFilterAND"]->Fill(5);
                if (isMC_) histocontainer1D_["eventFilterAND_wtd"]->Fill(5,weight_);
            }
            else return;

            if (debugMode_) std::cout<<"Passed Dimuon Cuts"<<std::endl;

            bool passHadronCuts {false};

            std::vector<int> chs;
            for (Int_t k = 0; k < numPackedCands; k++) {
                if (std::abs(packedCandsPdgId[k]) != 211) continue;
                if (packedCandsCharge[k] == 0 ) continue;
                if (packedCandsHasTrackDetails[k] != 1 ) continue;
                TLorentzVector lVec {packedCandsPx[k], packedCandsPy[k], packedCandsPz[k], packedCandsE[k]};
                if (lVec.Pt() < 0.5) continue;
                if (std::abs(lVec.Eta()) >= chEtaCut_) continue;
                if ( std::abs(lVec.Pt()) <= chPtCut_) continue;
                chs.emplace_back(k);
                if (chs.size()>=4) {
                    passHadronCuts=true;
                    break;
            ...
            // if (chs.size()>=2) passHadronCuts=true;
            chs.clear();
            if ((passGenCuts)&&(passHadronCuts)) {
                histocontainer1D_["eventFilterOR"]->Fill(6);
                // if (isMC_) histocontainer1D_["eventFilterOR_wtd"]->Fill(6,weight_);
            }
            if ((passGenCuts)&&(passTrigCuts)&&(passMetFilters)&&(passMuonCuts)&&(passDimuonCuts)&&(passHadronCuts)) { //no cut before this
                histocontainer1D_["eventFilterAND"]->Fill(6);
                if (isMC_) histocontainer1D_["eventFilterAND_wtd"]->Fill(6,weight_);
            }
            else return;

            if (debugMode_) std::cout<<"Passed Hadron Cuts"<<std::endl;


            bool passDihadronCuts {false};
            int dich_iters = 0;
            int idx1=-1;
            int idx2=-1;
            // int idx3=-1;
            ...
            for (Int_t k = 0; k < numPackedCands; k++) {
                ...
                for (Int_t l = k+1; l < numPackedCands; l++) {
                    if ((l==idx2) || (l==idx2)) continue;
                    if (std::abs(packedCandsPdgId[l]) != 211) continue;
                    if (packedCandsCharge[l] == 0 ) continue;
                    TLorentzVector ch1{packedCandsPx[k], packedCandsPy[k], packedCandsPz[k], packedCandsE[k]};
                    TLorentzVector ch2{packedCandsPx[l], packedCandsPy[l], packedCandsPz[l], packedCandsE[l]};
                    if (ch1.DeltaR(ch2) >= muoDRCut_) continue;
                    double invMass { (ch1+ch2).M() };
                    if (invMass <= invChChMassCutHi_)  {
                        dich_iters++;
                        idx1=k;
                        idx2=l;
                        if (dich_iters==2) {
                            passDihadronCuts = true;
                            break;
                        }
                    }
                }
                if ( passDihadronCuts ) break;
            }
            if (debugMode_) std::cout<<"No. of dihadron iters:"<<dich_iters<<std::endl;

            if ((passGenCuts)&&(passDihadronCuts)) {
                histocontainer1D_["eventFilterOR"]->Fill(7);
                // if (isMC_) histocontainer1D_["eventFilterOR_wtd"]->Fill(7,weight_);
            }
            if ((passGenCuts)&&(passTrigCuts)&&(passMetFilters)&&(passMuonCuts)&&(passDimuonCuts)&&(passHadronCuts)&&(passDihadronCuts)) { //no cut before this
                histocontainer1D_["eventFilterAND"]->Fill(7);
                if (isMC_) histocontainer1D_["eventFilterAND_wtd"]->Fill(7,weight_);
            }
            else return;

            if (debugMode_) std::cout<<"Passed DiHadron Cuts"<<std::endl;


            bool passMuMuhhCuts{true}; //not cutting on higgs mass at the moment
            int higgs_iters = 0;
            ...
            //         if (muon1.DeltaR(muon2) >= muoDRCut_) continue;
            //         double invMass { (muon1+muon2).M() };
            //         if (invMass > invMuMuMassCut_) continue;
            //         for (Int_t l = 0; l < numPackedCands; l++) {
            //             if (std::abs(packedCandsPdgId[l]) != 211) continue;
            //             if (packedCandsCharge[l] == 0 ) continue;
            //             if (packedCandsHasTrackDetails[l] != 1 ) continue;
                        
            //             TLorentzVector ch1{packedCandsPx[l], packedCandsPy[l], packedCandsPz[l], packedCandsE[l]};
            //             if (std::abs(ch1.Pt()) <= chPtCut_) continue;
            //             if (std::abs(ch1.Eta()) >= chEtaCut_) continue;
            //             for (Int_t m = l+1; m < numPackedCands; m++) {
            //                 higgs_iters++;
            //                 if (std::abs(packedCandsPdgId[m]) != 211) continue;
            //                 if (packedCandsCharge[m] == 0 ) continue;
            //                 if (packedCandsHasTrackDetails[m] != 1 ) continue;

            //                 TLorentzVector ch2{packedCandsPx[m], packedCandsPy[m], packedCandsPz[m], packedCandsE[m]};
            //                 if ( std::abs(ch2.Pt()) <= chPtCut_) continue;
            //                 if (std::abs(ch2.Eta()) >= chEtaCut_) continue;

            //                 if (ch1.DeltaR(ch2) >= muoDRCut_) continue;
            //                 double invMass_ch { (ch1+ch2).M() };
            //                 if (invMass_ch > invMuMuMassCut_) continue;
            //                 double invMass_higgs = (muon1+muon2+ch1+ch2).M();
            //                 if (invMass_higgs > invMuMuhhMassCut_) {
            //                     passMuMuhhCuts=true;
            //                     break;
            //                 }
            //             }
            //             if (passMuMuhhCuts) break;
            //         }
            //         if (passMuMuhhCuts) break;
            //     }
            //     if (passMuMuhhCuts) break;
            // }
            if (debugMode_) std::cout<<"No. of reco. higgs iters:"<<higgs_iters<<std::endl;

            if ((passGenCuts)&&(passMuMuhhCuts)) {
                histocontainer1D_["eventFilterOR"]->Fill(8);
                // if (isMC_) histocontainer1D_["eventFilterOR_wtd"]->Fill(7,weight_);
            }
            if ((passGenCuts)&&(passTrigCuts)&&(passMetFilters)&&(passMuonCuts)&&(passDimuonCuts)&&(passHadronCuts)&&(passDihadronCuts)&&(passMuMuhhCuts)) { //no cut before this
                histocontainer1D_["eventFilterAND"]->Fill(8);
                if (isMC_) histocontainer1D_["eventFilterAND_wtd"]->Fill(7,weight_);
            }
            else return;
            ...
            if ((passTrigCuts)&&(passMetFilters)&&(passMuonCuts)&&(passDimuonCuts)&&(passHadronCuts)&&(passDihadronCuts)&&(passMuMuhhCuts)) //no cut before this
                passAllCuts=true;
            else return;
            ...
            for (int j{0}; j < numMuo["PF"]; j++) {
                ...
                for (int k{j+1}; k < numMuo["PF"]; k++) {
                    ...
                    if (muon1.DeltaR(muon2) >= muoDRCut_) continue;
                    double invMass { (muon1+muon2).M() };
                    if (invMass > invMuMuMassCutHi_) continue;
                    for (Int_t l = 0; l < numPackedCands; l++) {
                        if (std::abs(packedCandsPdgId[l]) != 211) continue;
                        if (packedCandsCharge[l] == 0 ) continue;
                        if (packedCandsHasTrackDetails[l] != 1 ) continue;
                        ...
                        for (Int_t m = l+1; m < numPackedCands; m++) {
                            ...
                            if (ch1.DeltaR(ch2) >= muoDRCut_) continue;
                            double invMass_ch { (ch1+ch2).M() };
                            if (invMass_ch > invChChMassCutHi_) continue;
                            double invMass_higgs = (muon1+muon2+ch1+ch2).M();
                            if (invMass_higgs > invMuMuhhMassCut_) {
                                passMuMuhhCuts=true;
                                break;
...
void MakeTopologyNtupleMiniAOD_MuonPrefire::bookBranches() {
    ...
    // for (auto iTrig = triggerList_.begin(); iTrig != triggerList_.end();iTrig++)
    for (size_t iTrig{0}; iTrig < triggerList_.size(); iTrig++) {
...
//    mytree_->Branch("packedCandsTauIndex", &packedCandsTauIndex, "packedCandsTauIndex[numPackedCands]/I");
    ...
    mytree_->Branch("packedCandsPseudoTrkNumberOfPixelHits", &packedCandsPseudoTrkNumberOfPixelHits, "packedCandsPseudoTrkNumberOfPixelHits[numPackedCands]/I");
...
void MakeTopologyNtupleMiniAOD_MuonPrefire::endJob()
{
    ...
    std::cout << "number of events triggered by HLT_Mu50_v and not HLT_IsoMu27_v: " << trigCount << std::endl;
    std::cout << "\n+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+="
              << std::endl;
}
