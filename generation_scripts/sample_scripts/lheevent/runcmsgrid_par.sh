...
if [ "$produceWeights" == "true" ];
then 
    cp -p pwgevents${idx}.lhe pwgevents${idx}.lhe.orig
...
EOF
            ...
            mv pwgevents-rwgt${idx}.lhe pwgevents${idx}.lhe
...
cat pwgevents${idx}.lhe | grep -v "Random number generator exit values" > pwgevents${idx}.lhe.bkp
mv pwgevents${idx}.lhe.bkp pwgevents${idx}.lhe

xmllint --noout pwgevents${idx}.lhe > /dev/null 2>&1; test $? -eq 0 || fail_exit "xmllint integrity check failed on pwgevents${idx}.lhe"

cp pwgevents${idx}.lhe ${file}_final${idx}.lhe
...
if [ -s pwg-stat.dat ]; then
  ...
  echo "  "$XSECTION"   "$XSECUNC"  1.00000000000E-00 10001" >>  cmsgrid_final${idx}.lhe_F
